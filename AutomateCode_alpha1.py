...
def fechamento_criacao_usuario(nome_completo_usuario='', login_usuario_ad='',senha_temporaria = '', numero_registro_usuario='' ,dia='', numero_chamado='', mensagem_fechamento='', texto_arquivamento='', caminho_texto='',escolha_texto=''):
	...
	escolha_texto = input('Deseja mostrar o texto para colocar no chamado?: [S/N] ').strip().upper()[0]
	
	if escolha_texto == 'S' or escolha_texto == 's':
...
def administrador_login(nome_admin='', senha_admin='', lista_admin={}, opcao_logica1='', opcao_logica2='', opcao_logica3='', opcao_logica4=''):
	...
	'senha':'Valfenda@1moKlgdDH90WEYvi'
	}
	...
	if nome_admin == lista_admin['login'] and senha_admin == lista_admin['senha']:
		...
		while True:
			...
			[2] Fechamento chamado de instalação de software
			[3] Exit	
			''')
			sleep(1.5)
